...
{{ cookiecutter.description }}. Built with the [Agent-to-Agent (A2A) protocol](https://github.com/google/A2A), TypeScript, Express, and the Vercel AI SDK.
...
```mermaid
flowchart TD
    O["Orchestrator"] -- "A2A Protocol" --> A1
    O -- "A2A Protocol" --> A2
    O -. "A2A Protocol" .-> An

    A1["Agent #1<br/><em>(this template)</em>"] -- "MCP Protocol" --> M1["MCP Server #1<br/>(stdio)"]
    A1 -- "MCP Protocol" --> M2["MCP Server #2<br/>(HTTP)"]
    A1 -. "MCP Protocol" .-> Mn["MCP Server #N"]
